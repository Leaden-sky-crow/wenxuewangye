...
import type { ViewState, WorkType as WorkTypeEnum, Work, Collection } from '../types';
...
const WorkCard: React.FC<{ work: Work; onClick: () => void; }> = ({ work, onClick }) => (
    <div onClick={onClick} className={`bg-gray-900 p-6 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 cursor-pointer border border-gray-800 relative overflow-hidden flex flex-col ${work.isHidden ? 'opacity-70' : ''}`}>
...
export const CategoryView: React.FC<CategoryViewProps> = ({ category, setView }) => {
  ...
  const items = useMemo(() => {
    const works = dataService.getWorks()
      .filter(w => w.isPersonal && w.category === category && w.status === Status.Published)
          ...
          {items.map(item => 
            isCollection(item) ? (
              <CollectionCard 
                key={`col-${item.id}`}
                collection={item}
                onClick={() => setView({ page: 'collection', collectionId: item.id, workId: null })}
              />
            ) : (
              <WorkCard 
                key={`work-${item.id}`}
                work={item}
                onClick={() => setView({ page: 'detail', workId: item.id })}
              />
            )
          )}
        </div>
